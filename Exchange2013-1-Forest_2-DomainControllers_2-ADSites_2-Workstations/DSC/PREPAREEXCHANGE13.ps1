configuration PREPAREEXCHANGE13
{
   param
   (
    ...
    Node localhost
    {
        ...
        xRemoteFile dotNet47
        {
            DestinationPath = "S:\ExchangeInstall\NDP472-KB4054530-x86-x64-AllOS-ENU.exe"
            Uri             = "https://go.microsoft.com/fwlink/p/?linkid=863265"
            UserAgent       = "[Microsoft.PowerShell.Commands.PSUserAgent]::InternetExplorer"
            DependsOn = "[File]CreateSoftwareFolder"
        }

        Script Installdotnet47
        {
            SetScript =
            {
                # Install .Net 4.7
                S:\ExchangeInstall\NDP472-KB4054530-x86-x64-AllOS-ENU.exe /q
            }
            GetScript =  { @{} }
            TestScript = { $false}
            DependsOn = '[xRemoteFile]dotNet47'
        }

        xRemoteFile DownloadExchange
        {
            DestinationPath = "S:\ExchangeInstall\Exchange2013.ISO"
            Uri             = "$ExchangeSASUrl"
            UserAgent       = "[Microsoft.PowerShell.Commands.PSUserAgent]::InternetExplorer"
            DependsOn =    '[File]CreateSoftwareFolder'
        }
