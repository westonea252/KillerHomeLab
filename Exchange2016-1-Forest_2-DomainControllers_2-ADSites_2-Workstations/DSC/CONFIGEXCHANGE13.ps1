    ...
    Node localhost
    {
        ...
        Script ConfigureCertificate
        {
            SetScript =
            {
                ...
                $CertFile = Get-ChildItem -Path "C:\Certificates\owa2013.$using:RootDomainFQDN.pfx" -ErrorAction 0
                IF ($CertFile -eq $Null) {Get-ChildItem -Path cert:\LocalMachine\my\$thumbprint | Export-PfxCertificate -FilePath "C:\Certificates\owa2013.$using:RootDomainFQDN.pfx" -Password $Password}
        ...
        Script ConfigureExchange2013
        {
            SetScript =
            {
                ...
                Set-ClientAccessServer "$using:computerName" –AutodiscoverServiceInternalUri "https://autodiscover2013.$using:RootDomainFQDN/Autodiscover/Autodiscover.xml"
                
                # Set OWA Virtual Directory"
                Set-OWAVirtualDirectory –Identity "$using:computerName\owa (Default Web Site)" –InternalURL "https://owa2013.$using:RootDomainFQDN/OWA" -ExternalURL "https://owa2013.$using:RootDomainFQDN/OWA" -ExternalAuthenticationMethods NTLM -FormsAuthentication:$False -BasicAuthentication:$False –WindowsAuthentication:$True

                # Set ECP Virtual Directory
                Set-ECPVirtualDirectory –Identity "$using:computerName\ecp (Default Web Site)" –InternalURL "https://owa2013.$using:RootDomainFQDN/ECP" -ExternalURL "https://owa2013.$using:RootDomainFQDN/ECP" -ExternalAuthenticationMethods NTLM -FormsAuthentication:$False -BasicAuthentication:$False –WindowsAuthentication:$True

                # Set OAB Virtual Directory
                Set-OABVirtualDirectory –Identity "$using:computerName\oab (Default Web Site)" –InternalURL "https://outlook2013.$using:RootDomainFQDN/OAB" -ExternalURL "https://outlook2013.$using:RootDomainFQDN/OAB"
                ...
                Set-WebServicesVirtualDirectory –Identity "$using:computerName\EWS (Default Web Site)" –InternalURL "https://outlook2013.$using:RootDomainFQDN/EWS/Exchange.asmx" -ExternalURL "https://outlook2013.$using:RootDomainFQDN/EWS/Exchange.asmx"

                # Set ACTIVESYNC Virtual Directory
                Set-ActiveSyncVirtualDirectory –Identity "$using:computerName\Microsoft-Server-ActiveSync (Default Web Site)" –InternalURL "https://eas2013.$using:RootDomainFQDN/Microsoft-Server-ActiveSync" -ExternalURL "https://eas2013.$using:RootDomainFQDN/Microsoft-Server-ActiveSync"

                # Set MAPI Virtual Directory
                Set-MapiVirtualDirectory –Identity "$using:computerName\mapi (Default Web Site)" –InternalURL "https://outlook2013.$using:RootDomainFQDN/MAPI" -ExternalURL "https://outlook2013.$using:RootDomainFQDN/MAPI" -IISAuthenticationMethods Ntlm,OAuth,Negotiate
